...
function getWorldComponentType(worldType:Type, fieldName:String):Type {
    ...
    // Validate that the component type is explicitly typed
    validateComponentType(result.type, fieldName, result.pos);
...
/**
 * Validate that a component type is suitable for use in HxBitECS.
 *
 * This only checks for patterns that are definitely problematic:
 * - Generated hxbitecs types (like SoAx_y) - these indicate type inference issues
 *
 * Note: Anonymous types and arrays with anonymous elements are allowed, but they
 * won't work with @:wrapperUsing metadata. If you need static extensions or cleaner
 * wrapper type names, use an explicit typedef annotation:
 *   `var pos:Vector2 = new Vector2()` instead of `var pos = new Vector2()`
 */
function validateComponentType(componentType:Type, fieldName:String, fieldPos:Position):Void {
    switch componentType {
        // Abstract types might be generated by macros
        case TAbstract(t, _):
            var ref = t.get();
            var typeName = ref.name;
            var pack = ref.pack.join('.');

            // Check if it's a generated type from hxbitecs
            if (pack == 'hxbitecs') {
                Context.error(
                    'Component field "$fieldName" has inferred type "$typeName" from hxbitecs package. ' +
                    'This generated type cannot be used directly. ' +
                    'Use an explicit typedef: `var $fieldName:YourTypedef = ...`',
                    fieldPos
                );
            }
